    ...
    schemaReady = (async () => {
      ...
      await sql`
        CREATE TABLE IF NOT EXISTS settings (
          ...
          leagues TEXT NOT NULL DEFAULT '["Premier League","La Liga","Bundesliga","Serie A"]'
        )
      `;
      await sql`ALTER TABLE settings ADD COLUMN IF NOT EXISTS leagues TEXT NOT NULL DEFAULT '["Premier League","La Liga","Bundesliga","Serie A"]'`;
...
function rowToSettings(row) {
  ...
  return { hitRate: row.hit_rate, leagues };
}

export async function getSettings() {
  await ensureSchema();
  const sql = getSql();
  const rows = await sql`SELECT hit_rate, leagues FROM settings WHERE id = 1`;
  return rows.length > 0
    ? rowToSettings(rows[0])
    : { hitRate: "68%", leagues: ["Premier League", "La Liga", "Bundesliga", "Serie A"] };
}
...
export async function updateSettings(fields) {
  ...
  const hitRate = fields.hitRate !== undefined ? fields.hitRate : current.hitRate;
  const leagues = fields.leagues !== undefined ? fields.leagues : current.leagues;

  const rows = await sql`
    UPDATE settings SET hit_rate = ${hitRate}, leagues = ${JSON.stringify(leagues)}
    WHERE id = 1
    RETURNING hit_rate, leagues
  `;
